...
class MapAnalyzer(Node):
    ...
    def update_plot(self):
        """Update the live matplotlib plot with current grid and clusters."""
        ...
        for cluster in self._clusters:
            y = [y for y, _ in cluster]
            x = [x for _, x in cluster]
            self.ax.scatter(x, y, s=5, c='green', alpha=0.5)

        ry, rx = self.world_grid_coordinates(self.robot_position[0], self.robot_position[1])
        
        self.ax.scatter(rx, self.grid_height - 1 - ry, c='blue', s=50, alpha=0.5)

        if self.grid_path:
            gy, gx = self.grid_path[-1]
    ...
    def publish_cmd_vel(self):
        ...
        if relative_distance < self.DISTANCE_THRESHOLD: # Stop when below threshold
            self.world_path.pop(0) # Remove the coordinates when reached,
            # so that each function call it will move along the planned path
            twist_msg.angular.z = -1.0 # Stop turning completely
            twist_msg.linear.x = -1.6
            ...
            linear_speed = 0.6 * (1 - min(1.0, relative_distance / 0.5))  # Max speed at 0.5m away
            twist_msg.linear.x = max(0.3, linear_speed)  # Never go below 0.05 m/s
    ...
    def candidate_boundaries(self, grid):
        ...
        for cluster in clusters:
            ...
            # if len(cluster) <= 32 or len(cluster) >= 128:
            #     continue

            if len(cluster) <= 32:
                continue

            self.get_logger().info(f"Length of cluster: {len(cluster)}")
        ...
        # self.get_logger().info(f"Candidate boundaries: {candidate_boundaries}")
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        self.grid_path = self.breadth_first_search(self.grid, grid_position)
        self.world_path = [self.grid_world_coordinates(cell_y, cell_x) for (cell_y, cell_x) in self.grid_path]
...
def main(args=None):
    ...
    # Occupancy-grid generated by an LLM for optimal testing
    arr2 = np.array([
        # Top-left cluster: (1,1), (1,2), (1,3), (2,1), (2,2), (2,3)
        [-1, -1, -1, -1, -1, 100, 100, 100, 100, 100],
        [-1,  0,  0,  0, -1, 100, 100, 100, 100, 100],
        [-1,  0,  0,  0, -1, 100, 100, 100, 100, 100],
        [-1, -1, -1, -1, -1, 100, 100, 100, 100, 100],

        # Isolated edge cell at (4,5)
        [100, 100, 100, 100, 100,  0, 100, 100, 100, 100],
        [100, 100, 100, 100, 100, -1, 100, 100, 100, 100],

        # Non-edge free cells at (6,0), (6,1), (6,2)
        [ 0,  0,  0, 100, 100, 100, 100, 100, 100, 100],

        # Bottom-right cluster: (7,7), (7,8), (8,7), (8,8)
        [100, 100, 100, 100, 100, 100, -1,  0,  0, -1],
        [100, 100, 100, 100, 100, 100, -1,  0,  0, -1],
    ...
    # assert node.candidate_boundaries(arr2) == [(1, 1), (2, 1), (1, 2), (2, 2), (1, 3), (2, 3), (7, 7), (8, 7), (7, 8), (8, 8)], "Candidate boundaries are invalid"
    ...
    #
    #     # Define colors: -1 (red), 0 (white), 100 (black)
    #     colors = ['red', 'white', 'black']
    #     bounds = [-1.5, -0.5, 0.5, 100.5]  # Boundaries between values
    #     cmap = ListedColormap(colors)
    #     norm = BoundaryNorm(bounds, cmap.N)
    #
    #     plt.imshow(node.grid, cmap=cmap, norm=norm)
    #     plt.title("Occupancy Grid")
    #
    #     for cluster in node.glob_clusters:
    #         y = [y for y, _ in cluster]
    #         x = [x for _, x in cluster]
    #         plt.scatter(x, y)
    #
