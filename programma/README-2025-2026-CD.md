![logo](https://www.hu.nl/-/media/hu/afbeeldingen/algemeen/hu-logo.ashx) [](logo-id)

# Semester 4 van Technische Informatica[](title-id) <!-- omit in toc -->

### Inhoud[](toc-id) <!-- omit in toc -->

- [Een introductie](#een-introductie)
  - [Organisatie](#organisatie)
- [Programma](#programma)
  - [Onderwijsweek C2 (09-02-2026)](#onderwijsweek-c2-09-02-2026)
  - [Onderwijsweek C3 (16-02-2026)](#onderwijsweek-c3-16-02-2026)
  - [Onderwijsweek C4 (23-02-2026)](#onderwijsweek-c4-23-02-2026)
  - [Onderwijsweek C5 (02-03-2026)](#onderwijsweek-c5-02-03-2026)
  - [Onderwijsweek C6 (09-03-2026)](#onderwijsweek-c6-09-03-2026)
  - [Onderwijsweek C7 (16-03-2026)](#onderwijsweek-c7-16-03-2026)
  - [Onderwijsweek C8 (23-03-2026)](#onderwijsweek-c8-23-03-2026)
  - [Onderwijsweek C9 (30-03-2026)](#onderwijsweek-c9-30-03-2026)
  - [Onderwijsweek C10 (06-04-2026)](#onderwijsweek-c10-06-04-2026)
  - [Onderwijsweek D1 (13-04-2026)](#onderwijsweek-d1-13-04-2026)
  - [Onderwijsweek D2 (20-04-2026)](#onderwijsweek-d2-20-04-2026)
  - [Onderwijsweek D3 (27-04-2026)](#onderwijsweek-d3-27-04-2026)
  - [Onderwijsweek D4 (04-05-2026)](#onderwijsweek-d4-04-05-2026)
  - [Onderwijsweek D5 (11-05-2026)](#onderwijsweek-d5-11-05-2026)
  - [Onderwijsweek D6 (18-05-2026)](#onderwijsweek-d6-18-05-2026)
  - [Onderwijsweek D7 (25-05-2026)](#onderwijsweek-d7-25-05-2026)
  - [Onderwijsweek D8 (01-06-2026)](#onderwijsweek-d8-01-06-2026)
  - [Onderwijsweek D09 (08-06-2026)](#onderwijsweek-d09-08-06-2026)
  - [Onderwijsweek D10 (15-06-2026)](#onderwijsweek-d10-15-06-2026)
  - [Onderwijsweek D11 (22-06-2026)](#onderwijsweek-d11-22-06-2026)
  - [Onderwijsweek E1/D12 (29-06-2026)](#onderwijsweek-e1d12-29-06-2026)
  - [Onderwijsweek E2 (06-07-2026)](#onderwijsweek-e2-06-07-2026)


## Een introductie

### Organisatie

De contacturen zijn (subject to change):

|dag|van|tot| lokaal |
|---|---|---| --- |
|ma|09:00|12:00| Zie rooster |
|di|13:00|17:00| Zie rooster | 
|wo|13:00|17:00| Zie rooster |
|do|13:00|17:00| Zie rooster |

De docenten zijn:
- (**BB**) Bart Bozon
- (**HK**) Hasan Kurt
- (**TM**) Teun Pieter Modderman
- (**LB**) Len Bunte
- (**NG**) Nick Goris
- (**JH**) Jan Halsema
- (**GB**) Gerritjan Boshuizen 

De SLB'ers zijn:
- (**LB**) Len Bunte
- (**AD**) Adrie van Doesburg

## Programma

TODO
<br> [HBO-i Job Event](https://hboictjobevent.nl/)
| vr  | [IICT Carrièredag](https://husite.nl/hu-tech/nieuws/iictcarrieredag-oktober25/) |
**Vanaf 15:00 stage/INNO voorlichting** (Inge en Berend) <br> Zelfstandig werken aan project
ALDS support (1hr) (**BB**) 


### Onderwijsweek C2 (09-02-2026)

| Dag | Project & Kennissessie |
| --- |  --- |
| ma  |1)  Introductie Semester en Project (**HK , BB**) <br> 2) Bewustwording kwaliteiten (**BB**)  |
| di  | [Schrijven van sollicitatiebrief](../vermogens/solliciteren/README.md) (**LB**) |
| wo  | Kennismaking project, mogelijkheid tot stellen vragen aan klant (**HK, BB**) |
| do  | [Intro & Desktop-Environment (VSCode, CMake, Debugger)](../infrastructuur/desktop_environment/README.md)  (**??**) <br> :end: Einde sprint 0 & :runner: planning sprint |

### Onderwijsweek C3 (16-02-2026)

| Dag |          |
|-----|----------|
| ma  | Vakantie |
| di  | Vakantie |
| wo  | Vakantie |
| do  | Vakantie |
| vr  | Vakantie |

### Onderwijsweek C4 (23-02-2026)

| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [Introductie Gazebo](../hardware_simulatie/gazebo/README.md) <br>  [Teamrollen Belbin](https://werkenmetteamrollen.nl/vragenlijst/) (**BB**) |
| di  |[Hoe werken modellering/simulatie pakketten?](../hardware_simulatie/hoe-werken-modellering-en-simulatie-paketten/README.md)(**BB**) |
| wo  | [C++ Templates & Compiletime](../software/cpp/templates_en_compiletime.pdf) (**HK**) |
| do  | Feedback CV & Sollicatie brief (**GB , LB**) Teamindeling (**HK**) |

### Onderwijsweek C5 (02-03-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [Gazebo deel II ](../hardware_simulatie/gazebo/README.md#programma-gazebo-deel-ii) (**BB**)  |
| di  | [\[&\]( )->auto{} en STL](../software/cpp/lambdas_en_stl.pdf) (**HK**)  |
| wo  | Opzetten team inrichting (team contract, github,etc.) (**HK,BB,TM**) |
| do  | PROJECT TBD, IETS MET EEN TEAM OEFENING???? (**HK**)|

### Onderwijsweek C6 (09-03-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [PID control]()[Gazebo deel III](../hardware_simulatie/gazebo/README.md#programma-gazebo-deel-iii) (**BB**) |
| di  | [Computer Vision I (classical OpenCV))](../software/vision_en_AI/README.md#programma-vision-deel-i) (**BB**) |
| wo  | Einde sprint 1. Pitches (3min) per team aan klas, daarna gesprek met teamcoach. (**HK,BB**)  |
| do  | Planning sprint 2. Werken aan project (**HK,BB**)  |

### Onderwijsweek C7 (16-03-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [Intro deepdive](../vermogens/onderzoek/README.md) (**BB**)  & [Troubleshooting](../vermogens/troubleshooting/README.md) (**BB**) |
| di  | [Computer Vision II (object herkenning & intro neurale netwerken)](../software/vision_en_AI/README.md#programma-vision-deel-ii) (**BB**)|
| wo  | Ptrs, Ref, UB (**HK**) |
| do  | **[1e Peilmoment](../Verantwoordingsdocument_TI_S4.md)**   |

### Onderwijsweek C8 (23-03-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [Machine learning](../software/vision_en_AI/README.md#programma-machine-learning) (**BB**)  |
| di  | [Algoritmen & Datastructuren I (C++ / Python)](../software/algoritmen_en_datastructuren/README.md)(**BB**) |
| wo  | Einde sprint 2. Product demo per team (20 min) enkel aan PO voor vastleggen FR & NFR & KD etc. (**HK,BB**)  |
| do  | Git branches & (non) functional requirements (**TM**) Voorbereiding loopbaankeuzes (**LB**)|

### Onderwijsweek C9 (30-03-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [Machine learning II](../software/vision_en_AI/README.md#programma-machine-learning) werksessie (**BB**)                  |
| di  |   [ALDS II - data structuren](../software/algoritmen_en_datastructuren/README.md)(**BB**)
             |
| wo  |  project TODO (**BB**, **HK**)|
| do  |  Structural Design Patterns (**HK**)  |

### Onderwijsweek C10 (06-04-2026)

| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | [ALDS III (pathfinding)](../software/algoritmen_en_datastructuren/README.md#programma-alds-deel-3) (**BB**)
 |
| di  | Feedbacksessie onderzoek & projectdemo (**BB**)|
| wo  | Einde sprint 2. Product demo per teamcoach (10 min) aan klas van teamcoach (**HK,BB**) |
| do  | TD Creational/Behavioural design patterns ( **HK**) |

### Onderwijsweek D1 (13-04-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  |   [Value sensitive design workshop](../vermogens/Value_Sensitive_Design/README.md) (**BB**)|
| di  | Zelfstandig werken |
| wo  | project TODO (**BB**, **HK**)|
| do  | **2e Peilmoment**|

### Onderwijsweek D2 (20-04-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  |      |
| di  |  Zelfstandig werken  ()|
| wo  |10:00-12:00  Klassikale pitches I  & PO gesprekken  (**HK,BB**)|
| do  |   project TODO (**BB**, **HK**)  |




### Onderwijsweek D3 (27-04-2026)

| Dag |          |
|-----|----------|
| ma  | Vakantie |
| di  | Vakantie |
| wo  | Vakantie |
| do  | Vakantie |
| vr  | Vakantie |

### Onderwijsweek D4 (04-05-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | Deepdive (2) (**BB,HK**)|
| di  |  Zelfstandig werken  (**??**) <br> Vanaf 15:00 stagevoortgang bespreken |
| wo  |  What if workshop (**BB**)|
| do  |  project TODO (**BB**, **HK**) |

### Onderwijsweek D5 (11-05-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  |Deepdive (2) (**BB,HK**)  | 
| di  | Zelfstandig werken aan project (**BB**) |
| wo  |  project TODO (**BB**, **HK**)|
| do  | Zelfstandig werken aan project (**HK**)|

### Onderwijsweek D6 (18-05-2026)

| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | Deepdive (2) (**BB & NG**)  |
| di  | Zelfstandig werken aan project (**TM & AvD**)   |
| wo  |  project TODO (**BB**, **HK**)                                |
| do  | project TODO (**BB**, **HK**) |

### Onderwijsweek D7 (25-05-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | Deepdive (2) (**BB**)   |
| di  | Zelfstandig werken aan project (**TM & AvD**) |
| wo  |  project TODO (**BB**, **HK**) |
| do  | project TODO (**BB**, **HK**)|

### Onderwijsweek D8 (01-06-2026)
| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | Deepdive (2) (**BB**)    |
| di  | Zelfstandig werken aan project (**TM & AvD**) <br> Vanaf 15:00 stagevoortgang bespreken  |
| wo  |  project TODO (**BB**, **HK**) |
| do  |  project TODO (**BB**, **HK**) |

### Onderwijsweek D09 (08-06-2026)
| ma  | TODO   |
| di  | TODO   |
| wo  |  project TODO (**BB**, **HK**) |
| do  |  project TODO (**BB**, **HK**) |


### Onderwijsweek D10 (15-06-2026)

| Dag | Project & Kennissessie |
| --- |  --- |
| ma  | Deepdive verbetering (**BB**) |
| di  | Zelfstandig werken aan project (**TM & AvD**)|
| wo   Demo dag 12.00 uur – 15.00 uur (**HK & BB**)                        |
| do  | **4e Peilmoment** :end: Einde sprint 8 & planning afronden |


### Onderwijsweek D11 (22-06-2026)
De docenten bekijken deze week al het ingeleverde werk. Aan het eind van de week ontvang je individuele feedback en een eindbeoordeling.


### Onderwijsweek E1/D12 (29-06-2026)
Verbeterweek 1

Week E1 en E2 zijn er voor (mogelijke) verbeteringen van het ingeleverde werk of om binnen het semester verder de excelleren. Als niet alle leeruitkomsten Op Niveau of Boven Niveau zijn dan kan in samenspraak met de docent een (realistisch) plan worden gemaakt en voer je deze werkzaamheden in deze periode uit. Verder excelleren kan bijvoorbeeld door een leeruitkomst met verbeteringen van Op Niveau naar Boven Niveau te brengen.

### Onderwijsweek E2 (06-07-2026)
Verbeterweek 2

Week E1 en E2 zijn er voor (mogelijke) verbeteringen van het ingeleverde werk of om binnen het semester verder de excelleren. Als niet alle leeruitkomsten Op Niveau of Boven Niveau zijn dan kan in samenspraak met de docent een (realistisch) plan worden gemaakt en voer je deze werkzaamheden in deze periode uit. Verder excelleren kan bijvoorbeeld door een leeruitkomst met verbeteringen van Op Niveau naar Boven Niveau te brengen.
